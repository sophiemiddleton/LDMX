...
c--cuts
     ...
     $     etmax, emax, etamin, r2max, s_max, ptll_min, ptll_max, inclHtmin,inclHtmax
      double precision labpmax(nincoming+1:nexternal)
      double precision labpmin(nincoming+1:nexternal)
      double precision labanglemax(nincoming+1:nexternal)
      double precision labanglemin(nincoming+1:nexternal)
      double precision labptmax(nincoming+1:nexternal)
      double precision labptmin(nincoming+1:nexternal)
      common/to_labframe_cuts/ labpmin, labpmax, labanglemin, labanglemax, labptmin, labptmax
...
c For checking the consistency of the grouping and the cuts defined here
     ...
     $     ,labpmin_save(nexternal),labpmax_save(nexternal)
     $     ,labptmin_save(nexternal),labptmax_save(nexternal)
     $     ,labanglemin_save(nexternal),labanglemax_save(nexternal)
     ...
     $     ,etamin_save,etamax_save, labpmin_save, labpmax_save
     $     , labptmax_save, labptmin_save
     $     ,labanglemin_save, labanglemax_save
c
...
c        LEPTON
            if(is_a_l(i))then
                 ...
                 etamax(i)=etal
                 etamin(i)=etalmin
                 write(42,*) " in set cuts ",labplmax, " ", labplmin  
                 labpmin(i)=labplmin
 	             labpmax(i) =labplmax
...
c        PHOTON
            if(is_a_a(i))then
                 ...
                 etamin(i)=etaamin
                 labpmin(i)=labpamin
                 labptmin(i)=labptamin
 	             labptmax(i) =labptamax
                 labanglemax(i) = labangleamax
		 		 labanglemin(i) = labangleamin
            endif
c        QUARKONIUM
