...
C     EXTERNAL
C
      REAL*8 R2,DOT,ET,RAP,DJ,SumDot,pt,ALPHAS,PtDot, theta
      logical cut_bw,setclscales,dummy_cuts
      external R2,DOT,ET,RAP,DJ,SumDot,pt,ALPHAS,cut_bw,setclscales,PtDot,theta
      external dummy_cuts
C
C     GLOBAL
C
...
c jet cluster algorithm
      ...
      logical xgoode, xgoodf
      double precision iselectron
      double precision pq,qq,m,lf
      double precision labp(0:3, nexternal)
      double precision labangle(0:3, nexternal)
...
C $B$ DESACTIVATE_CUT $E$ !This is a tag for MadWeight
      ...
      if(debugCounter.lt.0) debug=.false.
...
c
c     calculate lab-frame momenta
c
      pLabFrame(0)=ebeam(1)+ebeam(2)
      pLabFrame(1)=0.0
      pLabFrame(2)=0.0
      pLabFrame(3)=sqrt(ebeam(1)**2-dot(p(0,1),p(0,1)))
     $             +sqrt(ebeam(2)**2-dot(p(0,2),p(0,2)))

      if(debug) write (*,*) "plabFrame: ",(pLabFrame(i), i=0,3)

      do j=1,nexternal
         call boostx(p(0,j),pLabFrame,labP(0,j))
         if(debug) write(*,*) "CM p ",j,(p(i,j), i=0,3)
         if(debug) write (*,*) "labp ",j,(labp(i,j), i=0,3)
      enddo



c     lab frame cuts : momentum (lepton)
      do i=nincoming+1,nexternal
         if(is_a_l(i)) then
            if(debug) write (*,*) 'labe(',i,')=',labp(0,i),'   ',labplmin,
     $           ':',labplmax
            if ((labp(0,i) .gt. labplmin) .AND.(labp(0,i) .lt. labplmax))then
                 write(43,*) labp(0,i)
            endif
            notgood=((labp(0,i) .lt. labplmin) .OR.
     $           (labplmax.gt.0.and.labp(0,i) .gt. labplmax))
            if(notgood) then
      ...
      enddo ! Close loop over i

c     lab frame cuts : momentum (photon)
      do i=nincoming+1,nexternal
         if(is_a_a(i)) then
            if(debug) write (*,*) 'labe(',i,')=',labp(0,i),'   ',labpamin
            notgood=(labp(0,i) .lt. labpamin) 
            if(notgood) then
               if(debug) write (*,*) i,' -> fails'
               passcuts=.false.
               return
      ...
      do i=nincoming+1,nexternal
         xgoodf = .false.
      	 ...
      	 if(is_a_a(i)) then
             ...
             labangle = theta(labp(0,i))
             if(
     $         (labangle(0,i) .gt. labangleamin) .AND. (labangle(0,i) .lt. labangleamax)) then
               xgoodf=.true. 
               passcuts = .true.   
               write(44,*), labangle(0,i)
            endif
            if(labangle(0,i) .lt. labangleamin) then
               if(debug) write(*,*) "photon to soft"
               passcuts=.false.
               return
            endif
            if(labangle(0,i) .gt. labangleamax) then
               if(debug) write(*,*) "photon too hard"
               passcuts=.false.
               return
            endif
